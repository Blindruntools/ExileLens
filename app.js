...
function parseItem(text) {
  ...
  for (const line of lines) {
    ...
    found.push({ label: matched.label, category: matched.cat, value, raw: line });
  }
  return found;
}
...
function updatePreview(textareaId, previewId) {
  ...
  preview.innerHTML = `<div class="stat-chips">${stats.map(s => `<span class="stat-chip" title="${s.raw}">+${s.value} ${s.label}</span>`).join('')}</div>`;
}
...
// ── TESSERACT CLIENT IMAGE OCR IMPLEMENTATION ──
document.querySelectorAll('.item-image-input').forEach(input => {
  input.addEventListener('change', function(e) {
    ...
    Tesseract.recognize(file, 'eng', { logger: m => console.log(m) })
      .then(({ data: { text } }) => {
        // Run standard string corrections on typical OCR scanning letter swaps
        let optimizedText = text
          .replace(/[lI|]/g, '1')    // Correct vertical pipeline read mismatches to absolute digits
          .replace(/o/gi, '0')       // Prevent character zero reading as text characters
...
function renderResults(scoreA, scoreB, statsA, statsB, build) {
  ...
  if (scoreA === 0 && scoreB === 0) {
    verdictText = 'Null Core'; verdictDesc = 'No readable parameters found.';
  } else if (scoreA === 0) {
    diffPct = 100; diffLabel = '% better'; diffClass = 'positive'; verdictClass = 'upgrade'; verdictIcon = '▲'; verdictText = 'Upgrade'; verdictDesc = 'Market target yields clear optimization.';
  } else if (scoreB === 0) {
    diffPct = 100; diffLabel = '% worse'; diffClass = 'negative'; verdictClass = 'downgrade'; verdictIcon = '▼'; verdictText = 'Downgrade'; verdictDesc = 'Retain equipped element.';
  } else {
    ...
    if (raw > 10) {
      ...
      verdictClass = 'sidegrade'; verdictIcon = '◈'; verdictText = 'Sidegrade'; verdictDesc = 'Negligible variance. Balance based on resistance needs.';
    }
  }
  ...
  inner.innerHTML = `
    ...
    <div class="scores-grid">
      <div class="score-card">
        ...
        <div class="stat-breakdown">${statsA.map(s => `<div class="breakdown-row"><span class="breakdown-stat">${s.label}</span><span class="breakdown-val">${s.value}</span></div>`).join('') || 'No valid mods.'}</div>
      </div>
      <div class="score-card">
        ...
        <div class="stat-breakdown">${statsB.map(s => `<div class="breakdown-row"><span class="breakdown-stat">${s.label}</span><span class="breakdown-val">${s.value}</span></div>`).join('') || 'No valid mods.'}</div>
      </div>
    </div>
